...
class DSCCNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 22, (22, 1))
        self.Bn1 = nn.BatchNorm2d(22)
        self.conv2 = nn.Conv3d(22, 20, (2, 1, 12), padding=(0, 0, 6))
        self.Bn2   = nn.BatchNorm2d(20)
        self.SquareLayer = square_layer()
        self.Drop1 = nn.Dropout(0.5)
        self.AvgPool1 = nn.AvgPool2d((1, 62), stride=(1, 12))
    ...
    def forward(self, x):
        ...
        l, r = self.Bn1(l), self.Bn1(r)
        l, r = torch.unsqueeze(l, 2), torch.unsqueeze(r, 2)
        out = torch.cat((l, r), 2)
...
def getDataLoader(subject):
    ...
    # train_X = np.concatenate((tmp_X[0, :12], tmp_X[1, :12], tmp_X[2, :12], tmp_X[3, :12]), axis = 0)
    # train_y = np.concatenate((tmp_y[0, :12], tmp_y[1, :12], tmp_y[2, :12], tmp_y[3, :12]), axis = 0)
    print(train_X.shape)
    print(train_y.shape)
...
def train(net, train_dataloader, validate_dataloader, subject, num, write, epochs=10000):
    ...
    num = f"num_{num}_lr5e-4_bs4096_drp5_0"
    if write:
        writer = SummaryWriter("experiment/aug19/" + name + '/' + num + '/')
